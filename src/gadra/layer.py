...
class GaDRALinear(nn.Module, BaseTunerLayer):
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        if gate not in ("hard", "soft", "none"):
            raise ValueError(f"gate must be 'hard', 'soft', or 'none', got: {gate}")
    ...
    def reset_adapter_parameters(self, adapter_name: str) -> None:
        ...
        if bias_init is not None and adapter_name in self.gadra_gate:
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        ...
        for active_adapter in self.active_adapters:
            ...
            if gate_kind == "none":
                ...
                if self.router_conditioning[active_adapter] == "dual":
                ...
                gamma = compute_gamma(
                    ...
                    gamma_threshold=self.gamma_threshold[active_adapter],
...
class GaDRAParallelBlock(nn.Module, BaseTunerLayer):
    """GaDRA gated residual adapter wrapped in parallel to a whole block (e.g. an MLP/MoE block).

        out      = base_block(x)                 # x, out[0] : [*, H]  (tuple-returning blocks supported)
        delta    = (lora_alpha / r) * gadra_B(gadra_A(dropout(x)))   # gadra_A: H->r, gadra_B: r->H
        gate_in  = delta (mono) | cat(out, delta) (dual)
        gamma    = compute_gamma(gadra_gate(dropout(gate_in)), ...)  | 1.0 (gate="none")
        return out + gamma * delta               # tuple blocks: (out[0]+gamma*delta, *out[1:])

    Non-mergeable. ``gate="none"`` is the ungated LoRA-parallel baseline (no ``gadra_gate``, gamma=1).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # The base is a composite block (no in/out features); the adapter runs at the residual-stream dim H.
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        h = self.hidden_size
        self.lora_dropout[adapter_name] = nn.Dropout(p=lora_dropout) if lora_dropout > 0.0 else nn.Identity()
        self.gadra_A[adapter_name] = nn.Linear(h, r, bias=False)
        self.gadra_B[adapter_name] = nn.Linear(r, h, bias=False)
        if gate != "none":
            gate_in_dim = 2 * h if router_conditioning == "dual" else h
            self.gadra_gate[adapter_name] = nn.Linear(gate_in_dim, 1, bias=True)
    ...
    def merge(self, safe_merge: bool = False, adapter_names: Optional[list[str]] = None) -> None:
        raise NotImplementedError(
            "GaDRA is non-mergeable: gamma is a per-token, input-dependent gate and the adapter is "
            "parallel to a composite block, so it cannot be folded into a base weight."
        )
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        base_out, rest = (out[0], out[1:]) if isinstance(out, tuple) else (out, ())
        result_dtype = base_out.dtype
        result = base_out
