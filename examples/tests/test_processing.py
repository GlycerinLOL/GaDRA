"""Data-path parity: ``examples.processing`` reproduces the research repo's packed token stream."""
...
def test_packing_collator_matches_golden(golden):
    """Packing layout matches the golden bit-for-bit."""
...
def test_group_texts_matches_golden(golden):
    """group_texts output matches the golden."""
    feats = {
...
def test_tokenize_pt_and_eos_match_golden(golden):
    """tokenize_pt and EOS detection reproduce the parent per-sample token ids."""
